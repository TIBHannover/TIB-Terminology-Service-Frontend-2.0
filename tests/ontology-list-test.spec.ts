...
test("ontology list is loaded", async ({ page }) => {
  await page.goto(
    BASE_URL + "/ontologies?and=false&sortedBy=title&page=1&size=10",
  );
...
test("ontology list result count is correct", async ({ page }) => {
  const size = 20;
  await page.goto(
    BASE_URL + `/ontologies?and=false&sortedBy=title&page=1&size=${size}`,
  );
...
test("ontology list filter by keyword in URL", async ({ page }) => {
  ...
  await page.goto(
    BASE_URL +
      `/ontologies?and=false&sortedBy=title&page=1&size=10&keyword=${keyword}`,
  );
  const facetRow = await page.locator("#ontologylist-search-grid");
  const filterInput = await facetRow.locator("input");
...
test("ontology list result collection facet filter from URL", async ({
  ...
  await page.goto(
    BASE_URL +
      `/ontologies?and=false&sortedBy=title&page=1&size=10&collection=${COLLECTION_ID_TO_TEST}`,
  );
  const expectedResultCount = await page
    .locator(`#facet-count-${COLLECTION_ID_TO_TEST}`)
    .innerHTML();
