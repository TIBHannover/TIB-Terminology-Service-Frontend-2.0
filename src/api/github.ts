...
export async function getOntologyGithubIssueList(
  ...
  try {
    ...
    let endpoint =
      process.env.REACT_APP_MICRO_BACKEND_ENDPOINT + "/github/issuelist?";
...
export async function getGitRepoTemplates(props: {
  repoUrl: string;
  gitUsername: string;
}) {
  try {
...
export async function submitGitIssue(props: {
  repoUrl: string;
  gitUsername: string;
  issueTitle: string;
  issueBody: string;
  ...
  try {
    ...
    let { repoUrl, gitUsername, issueTitle, issueBody, issueType, ontologyId } =
      props;
    let data = {
    ...
    let url =
      process.env.REACT_APP_MICRO_BACKEND_ENDPOINT + "/github/submit_issue/";
    let resp = await fetch(url, {
      method: "POST",
      headers: headers,
