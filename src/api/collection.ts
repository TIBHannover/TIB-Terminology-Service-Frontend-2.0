...
export function getCollectionStatFromOntoList(ontoList: TsOntology[]): {
  [key: string]: number;
} {
...
export async function getCollectionsAndThierOntologies(): Promise<{
  [key: string]: TsOntology[];
}> {
...
export async function getBioregistryCollection(
  collectionName: string,
): Promise<BioregistryCollection> {
  type Resp = {
    _result: {
  ...
  try {
    let result = await fetch(
      process.env.REACT_APP_MICRO_BACKEND_ENDPOINT +
        "/collection/bioregistry_collections/",
    );
    if (!result.ok) {
      return {};
    }
