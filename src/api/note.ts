  ...
  try {
    ...
    await Promise.all(
      notes["notes"].map(async (note) => {
        if (
          !note["semantic_component_label"] &&
          note["semantic_component_type"] !== "ontology"
        ) {
          let termApi: any = new TermApi(
            note["ontology_id"],
            note["semantic_component_iri"],
            note["semantic_component_type"],
          );
...
): Promise<NoteDetailResponse | {} | "404"> {
  try {
