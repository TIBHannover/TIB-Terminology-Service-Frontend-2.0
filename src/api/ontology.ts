...
class OntologyApi {
  ...
  async fetchRootClasses(): Promise<{
    roots: TsClass[];
    obsoletes: TsClass[];
  }> {
    try {
  ...
  async fetchRootProperties(): Promise<{
    roots: TsProperty[];
...
export async function runShapeTest(
  ontologyPurl: string,
): Promise<OntologyShapeTestResult | boolean> {
  try {
...
export async function submitOntologySuggestion(
...
export async function checkOntologyPurlIsValidUrl(
  purl: string,
): Promise<OntologyPurlValidationRes> {
  try {
...
export async function submitAdopterRequest(formData: any): Promise<boolean> {
  try {
    ...
    let url =
      process.env.REACT_APP_MICRO_BACKEND_ENDPOINT +
      "/ontologysuggestion/adopter_create/";
