...
export async function olsSearch(
  ...
  try {
    ...
    let ontologies = inputData.selectedOntologies?.map((oId) =>
      oId.toLowerCase(),
    );
    let searchUrl: string =
      apiBaseUrl +
      `/v2/entities?search=${query}&page=${page}&size=${size}&lang=${lang}&exclusive=true`;
    searchUrl =
      !inputData.includeImported && !inputData.fromOntologyPage
        ? searchUrl + "&isDefiningOntology=true"
        : searchUrl;
    // searchUrl = jumpToMode ? (searchUrl + "&boostFields=label^3") : searchUrl;
    searchUrl = !jumpToMode
      ? searchUrl + "&facetFields=type+ontologyId"
      : searchUrl;
    searchUrl = ontologies?.length
      ? searchUrl + `&ontology=${ontologies.join(",")}`
      : searchUrl;
    searchUrl = inputData?.selectedTypes?.length
      ? searchUrl + `&type=${inputData?.selectedTypes?.join(",")}`
      : searchUrl;
    searchUrl = inputData?.searchInValues?.length
      ? searchUrl + `&searchFields=${inputData?.searchInValues?.join("+")}`
      : searchUrl;
    searchUrl = inputData?.searchUnderIris?.length
      ? searchUrl + `&childrenOf=${inputData?.searchUnderIris?.join(",")}`
      : searchUrl;
    searchUrl = inputData?.searchUnderAllIris?.length
      ? searchUrl + `&allChildrenOf=${inputData?.searchUnderAllIris?.join(",")}`
      : searchUrl;
    searchUrl = inputData.obsoletes
      ? searchUrl + "&includeObsoleteEntities=true"
      : searchUrl;
    ...
    if (
      ...
      inputData?.selectedCollections?.length
    ) {
...
export async function getJumpToResult(
  inputData: SuggestAndSelectApiInput,
  count: number = 10,
  lang: string = "en",
): Promise<BaseSearchSingleResult[]> {
  try {
    inputData["searchUnderIris"] = inputData["searchUnderIris"]
      ? inputData["searchUnderIris"]
      : [];
    inputData["searchUnderAllIris"] = inputData["searchUnderAllIris"]
      ? inputData["searchUnderAllIris"]
      : [];
    let autocompleteApiBaseUrl: string = process.env.REACT_APP_SEARCH_URL || "";
    autocompleteApiBaseUrl =
      autocompleteApiBaseUrl.split("search")[0] + "select";
    let url = `${autocompleteApiBaseUrl}?q=${encodeURIComponent(inputData["searchQuery"])}&rows=${count}&lang=${lang}`;
    url = inputData["ontologyIds"]
      ? url + `&ontology=${inputData["ontologyIds"]}`
      : url;
    url = inputData["types"] ? url + `&type=${inputData["types"]}` : url;
    url = inputData["obsoletes"] ? url + "&obsoletes=true" : url;
    url =
      inputData["searchUnderIris"].length !== 0
        ? url + `&childrenOf=${inputData.searchUnderIris.join(",")}`
        : url;
    url =
      inputData["searchUnderAllIris"].length !== 0
        ? url + `&allChildrenOf=${inputData.searchUnderAllIris.join(",")}`
        : url;
    url = inputData["collectionIds"]
      ? url + `&schema=collection&classification=${inputData["collectionIds"]}`
      : url;
...
export async function getAutoCompleteResult(
  inputData: SuggestAndSelectApiInput,
  count: number = 5,
): Promise<AutoSuggestSingleResult[]> {
  try {
    let lang = Toolkit.getVarInLocalSrorageIfExist("language", "en");
    let url: string =
      process.env.REACT_APP_API_URL +
      `/suggest?q=${encodeURIComponent(inputData["searchQuery"])}&rows=${count}&lang=${lang}`;
    url = inputData["ontologyIds"]
      ? url + `&ontology=${inputData["ontologyIds"]}`
      : url;
    url = inputData["types"] ? url + `&type=${inputData["types"]}` : url;
