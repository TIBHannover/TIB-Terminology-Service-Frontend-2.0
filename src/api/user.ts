...
export async function fetchSearchSettings(): Promise<
  Array<SearchSettingApiResponse> | []
> {
  try {
    let headers: TsPluginHeader = getTsPluginHeaders({
      isJson: false,
...
export async function updateSearchSettings(
  ...
  try {
    ...
    let result: any = await fetch(
      baseUrl + "/user/search_setting/" + settingId + "/",
      {
        method: "PUT",
        headers: headers,
        body: JSON.stringify(settingData),
...
export async function deleteSearchSetting(
  settingId: string | number,
): Promise<boolean> {
  try {
    let headers: TsPluginHeader = getTsPluginHeaders({
