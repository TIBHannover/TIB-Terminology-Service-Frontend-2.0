...
export async function getUserTermsetList(userId: string): Promise<TsTermset[]> {
  try {
    ...
    let userTermSets: TermSet[] = termSetList["_result"]["term_sets"].filter(
      (tset) => tset.creator?.id === userId,
    );
    userTermSets.sort((s1, s2) => s1.name.localeCompare(s2.name));
    let tsTermsets: TsTermset[] = [];
    for (let ts of userTermSets) {
...
export async function createTermset(
  termset: NewTermSetFormData,
): Promise<TsTermset | null> {
  try {
    type RespType = {
      _result: {
        term_set: TermSet;
    ...
    let url =
      process.env.REACT_APP_MICRO_BACKEND_ENDPOINT + "/term_set/create/";
...
export async function addTermToMultipleSets(
  setIds: string[],
  ...
  try {
    ...
    return results.reduce(
      (lastCalculation: boolean, current: RespType) =>
        lastCalculation && current["_result"]["added"],
      true,
    );
  } catch (e) {
    return false;
...
export async function removeTermFromSet(
  termsetId: string,
  termId: string,
): Promise<boolean> {
  try {
