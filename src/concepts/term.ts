...
export class TsTerm {
  ...
  static ANNOTATION_EXPECTION = [
    ...
    TsTerm.CURATION_STATUS_PURL,
    TsTerm.DB_XREF_PURL,
    TsTerm.PROPERTY_DOMAIN_PURL,
  ...
  constructor(termData: OntologyTermDataV2) {
    this.term = termData;
    this._id =
      termData.iri + "___" + Math.random().toString(36).substring(2, 20);
  }

  get type(): string {
    if (this.term.type && this.term.type.length > 0) {
  ...
  get label(): string {
    try {
      ...
      let label = this.term.label[this.term.label.length - 1] as
        | string
        | { value: string };
