...
const AddCollection = (props) => {
  const {
    ...
    editBtnText,
    btnClass,
    exstingCollectionList,
    ontologiesListForSelection,
  } = props;
  ...
  function onTextInputChange(e) {
    ...
    document.getElementById("max-char-message" + idPostfix).style.color =
      "black";
    setShowAlert(false);
  }

  function returnCollectionTitleIfValid() {
    let collectionTitle = document.getElementById(
      "collectionTitle" + idPostfix,
    ).value;
    ...
    if (!collectionTitle || collectionTitle === "") {
      document.getElementById("collectionTitle" + idPostfix).style.border =
        "1px solid red";
      return false;
    }
  ...
  async function saveNewCollection() {
    ...
    if (selectedOntologies.length === 0) {
      document.getElementById(
        "collection-ontologies" + idPostfix,
    ...
    setShowAlert(false);
    let collectionDescription = document.getElementById(
      "collectionDescription" + idPostfix,
    ).value;
    let ontologyIds = [];
  ...
  const modalId = editMode
    ? "editCollectionModal" + idPostfix
    : "newCollectionModal";
  return (
    <>
      <button
      ...
      <Modal show={showModal} id={modalId}>
        <Modal.Header>
          <h5 className="modal-title" id={modalId + "Label"}>
            {!editMode ? "New Collection" : "Edit Collection"}
          </h5>
        </Modal.Header>
        <Modal.Body>
          {showAlert && (
            <AlertBox type="danger" message="Collection name already exists." />
          )}
          <div className="row">
            <div className="col-sm-12">
              <label
                className="required_input"
                htmlFor={"collectionTitle" + idPostfix}
              >
                Name
              </label>
              <input
          ...
          <div className="row">
            <div className="col-sm-12">
              <label
                className="required_input"
                htmlFor={"collection-ontologies" + idPostfix}
              >
                Ontologies
              </label>
              {ontologiesListForSelection &&
          ...
          <div className="row">
            <div className="col-sm-12">
              <label htmlFor={"collectionDescription" + idPostfix}>
