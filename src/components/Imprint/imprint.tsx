import "../layout/documentationPage.css";

const Imprint = () => {
  return (
    <div className="row justify-content-center doc-design">
      <div className="col-sm-8">
        <h2>Imprint</h2>
        <p>
          Imprint for this website &#8208; also serves as provider
          identification according to &sect; 5 Digitale Dienste Gesetz (DDG)
        </p>
        <br />
        <h3>Provider:</h3>
        <p>
          Technische Informationsbibliothek (TIB)
          <br />
          Welfengarten 1 B, 30167 Hannover
          <br />
          Postfach 6080, 30060 Hannover
          <br />
        </p>
        <br />
        <br />
        <h3>Authorised Representative:</h3>

        <p>
          Prof. Dr. S&ouml;ren Auer (Director of TIB)
          <br />
          Technische Informationsbibliothek (TIB) is a foundation of public law
          of the state of Lower Saxony.
          <br />
        </p>
        <br />
        <h3>Responsible Supervisory Authority:</h3>

        <p>Ministry for Science and Culture of Lower Saxony</p>

        <h3>Contact:</h3>

        <p>
          Customer service phone: +49 511 762-8989
          <br />
          Central information desk phone: +49 511 762-2268
          <br />
          Fax: +49 511 762-4076
          <br />
          Email: <a href="mailto:information@tib.eu">information@tib.eu</a>
          <br />
        </p>

        <h3>VAT (sales tax) registration number:</h3>

        <p>DE 214931803</p>

        <h3>Editorial Office:</h3>

        <p>
          Axel Klinger
          <br />
          Email: <a href="mailto:axel.klinger@tib.eu">axel.klinger@tib.eu</a>
          <br />
        </p>

        <h3>Copyright Notice:</h3>

        <p>
          The layout of the homepage, the graphics used and the other content
          are protected by copyright.
          <br />
        </p>
      </div>
    </div>
  );
};

export default Imprint;
