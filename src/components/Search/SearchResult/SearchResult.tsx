...
const SearchResult = () => {
  ...
  let language =
    ...
    Toolkit.getVarInLocalSrorageIfExist("language", false) ||
    "en";
  ...
  const [selectedOntologies, setSelectedOntologies] = useState(
    SearchLib.getFilterAndAdvancedOntologyIdsFromUrl(),
  );
  const [selectedTypes, setSelectedTypes] = useState(searchUrlFactory.types);
  const [selectedCollections, setSelectedCollections] = useState(
    searchUrlFactory.collections,
  );
  const [facetFields, setFacetFields] = useState<SearchResultFacet>({
    type: {},
    ontologyId: {},
  });
  const [pageNumber, setPageNumber] = useState(
    parseInt(
      searchUrlFactory.page ? searchUrlFactory.page : DEFAULT_PAGE_NUMBER,
    ),
  );
  const [pageSize, setPageSize] = useState(
    parseInt(searchUrlFactory.size ? searchUrlFactory.size : DEFAULT_PAGE_SIZE),
  );
  ...
  const [obsoletes, setObsolete] = useState(
    searchUrlFactory.obsoletes
      ? searchUrlFactory.obsoletes === "true"
  ...
  async function search() {
    ...
    try {
      ...
      olsSearch(searchParams).then((result) => {
        ...
        setSearchResult(result["elements"] as Array<TsTerm>);
        setLoading(false);
        result["facetFieldsToCounts"]["type"] =
          result["facetFieldsToCounts"]["type"];
        result["facetFieldsToCounts"]["ontologyId"] =
          result["facetFieldsToCounts"]["ontologyId"];
        setTotalResultsCount(result.totalElements);
        setFacetFields(result.facetFieldsToCounts);
      });
  ...
  function setResultTitleAndLabel(resultItem: TsTerm, obsoletes: boolean) {
    let content = [];
    let targetHref =
      process.env.REACT_APP_PROJECT_SUB_PATH +
      "/ontologies/" +
      encodeURIComponent(resultItem.ontologyId);
    let termType = resultItem.type;
    if (termType === "class") {
      targetHref +=
        "/terms?iri=" +
        encodeURIComponent(resultItem.iri) +
        `&obsoletes=${obsoletes}`;
    } else if (
      [
        "property",
    ...
    content.push(
      <div className="search-card-title">
        <Link to={targetHref} className="search-result-title">
          [{termType}] <h4>{resultItem.label}</h4>
        </Link>
        ...
        <div className="row result-card" key={searchResult[i].curie}>
          <div className="col-sm-10">
            ...
            <div className="searchresult-iri">
              {searchResult[i].iri}
              <CopyLinkButton
                valueToCopy={searchResult[i].iri}
              ></CopyLinkButton>
            </div>
  ...
  function handlePageSizeDropDownChange(
  ...
  function handleCollectionFacetSelection(
  ...
  function clearFilters() {
    let allFacetCheckBoxes = [
      ...document.getElementsByClassName("search-facet-checkbox"),
    ] as HTMLInputElement[];
    for (let checkbox of allFacetCheckBoxes) {
      if (checkbox.dataset.ischecked !== "true") {
        (document.getElementById(checkbox.id)! as HTMLInputElement).checked =
          false;
      }
  ...
  function handleRemoveTagClick(e: React.MouseEvent<HTMLElement>) {
    try {
      ...
      localStorage.setItem("language", "en");
      commonUrlFactory.deleteParam({ name: SiteUrlParamNames.Lang });
      commonUrlFactory.deleteParam({
        name: SiteUrlParamNames.FromOntologyPage,
      });
      setLang("en");
    } catch (e) {
      // console.info(e);
  ...
  function createFilterTags() {
    ...
    for (let type of selectedTypes) {
      let newTag = (
        <div className="search-filter-tags" key={type}>
          {type}{" "}
          <i
            onClick={handleRemoveTagClick}
            data-type={"type"}
            data-value={type}
            className="fa fa-close remove-tag-icon"
          ></i>
        </div>
      );
    ...
    for (let ontologyId of selectedOntologies) {
      let newTag = (
        <div className="search-filter-tags" key={ontologyId}>
          {ontologyId}{" "}
          <i
            onClick={handleRemoveTagClick}
            data-type={"ontology"}
    ...
    for (let collection of selectedCollections) {
      let newTag = (
        <div className="search-filter-tags" key={collection}>
          {collection}{" "}
          <i
            onClick={handleRemoveTagClick}
  ...
  useEffect(() => {
    ...
    setObsolete(
      newSearchUrlFactory.obsoletes
        ? newSearchUrlFactory.obsoletes === "true"
        : Toolkit.getObsoleteFlagValue(),
    );
  ...
  return (
    <div
      className="row justify-content-center search-result-container"
      id="searchterm-wrapper"
    >
