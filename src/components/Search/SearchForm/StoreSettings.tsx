...
const StoreUpdateSearchSetting = (props) => {
  ...
  function returnSettingTitleIfValid() {
    ...
    if (!title || title === "") {
      document.getElementById("searchSettingTitle").style.border =
        "1px solid red";
      return false;
    }
    return title;
  ...
  async function update() {
    ...
    let response = await updateSearchSettings(
      searchSettingDataInStore.id,
      settingData,
    );
    let userSettings = { ...appContext.userSettings };
  ...
  return (
    <>
      {appContext.userSettings.activeSearchSetting.setting !== undefined && (
        <button className="btn btn-secondary ms-2" onClick={update}>
          Update
        </button>
      )}
      {appContext.userSettings.activeSearchSetting.setting === undefined && (
      ...
      <Modal show={modalIsOpen} id={"storeSearchSettingModal"}>
        <Modal.Header className="modal-header">
          <h5 className="modal-title" id={"storeSearchSettingModalLabel"}>
            {"Store search settings"}
          </h5>
        </Modal.Header>
        <Modal.Body className="modal-body">
          {showAlert && (
            <AlertBox type="danger" message="Setting name already exists." />
          )}
          <SearchSettingForm editMode={false} />
        </Modal.Body>
        <Modal.Footer className="modal-footer">
          ...
          <button type="button" className="btn btn-secondary" onClick={store}>
            Save
          </button>
        </Modal.Footer>
      </Modal>
...
export const SearchSettingForm = (props) => {
  ...
  useEffect(() => {
    if (editMode && settingToEdit) {
      document.getElementById("searchSettingTitle").value =
        settingToEdit["title"];
      document.getElementById("searchSettingDescription").value =
        settingToEdit["description"];
    }
  }, []);
    ...
    <>
      <div className="row">
        <div className="col-sm-12">
          <label className="required_input" htmlFor={"searchSettingTitle"}>
            Name
          </label>
          <input
            type="text"
      ...
      <div className="row">
        <div className="col-sm-12">
          <label htmlFor={"searchSettingDescription"}>
