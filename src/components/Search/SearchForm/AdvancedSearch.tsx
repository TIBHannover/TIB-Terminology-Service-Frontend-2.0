...
const AdvancedSearch = (props) => {
  ...
  const [selectedMetaData, setSelectedMetaData] = useState(
    SearchLib.getSearchInMetadataFieldsFromUrl(),
  );
  const [selectedSearchUnderTerms, setSelectedSearchUnderTerms] = useState(
    SearchLib.getSearchUnderTermsFromUrl(),
  );
  const [selectedSearchUnderAllTerms, setSelectedSearchUnderAllTerms] =
    useState(SearchLib.getSearchUnderAllTermsFromUrl());
  const [termListForSearchUnder, setTermListForSearchUnder] = useState([]);
  const [loadingResult, setLoadingResult] = useState(true);
  const [placeHolderExtraText, setPlaceHolderExtraText] = useState(
    createOntologyListForPlaceholder([]),
  );
  ...
  async function loadTermsForSelection(query) {
    ...
    for (let term of terms) {
      let opt = {};
      opt["text"] =
        `${term["ontology_prefix"]}:${term["label"]} (${term["type"]})`;
      opt["iri"] = term["iri"];
      options.push(opt);
    }
  ...
  function handleClickOutsideSelectionBox(e) {
    ...
    let advSearchUnderAllBox = document.getElementById(
      "adv-s-search-under-all-term",
    );
    if (
      !advSearchUnderBox?.contains(e.target) &&
      !advSearchUnderAllBox?.contains(e.target)
    ) {
      setTermListForSearchUnder([]);
      setLoadingResult(true);
    }
  ...
  useEffect(() => {
    document.addEventListener(
      "mousedown",
      handleClickOutsideSelectionBox,
      true,
    );
  ...
  useEffect(() => {
    if (
      ...
      setSelectedMetaData(
        appContext.userSettings.activeSearchSetting?.setting?.selectedMetaData,
      );
      setSelectedSearchUnderTerms(
        appContext.userSettings.activeSearchSetting?.setting
          ?.selectedSearchUnderTerms,
      );
      setSelectedSearchUnderAllTerms(
        appContext.userSettings.activeSearchSetting?.setting
      ...
      setSearchSettingIsModified(
        appContext.userSettings.activeSearchSettingIsModified,
      );
    }
            ...
            <div className="row">
              <div className="col-sm-12">
                <div className="row">
                  <div
                    className={
                      "col-sm-12 adv-search-label-holder " +
                      (searchSettingIsModified && "warning-text")
                    }
                  >
                    {loadedSettingName && (
                      <h5>
                        Loaded:{" "}
                        {loadedSettingName +
                          (searchSettingIsModified ? " *" : "")}
                        <small>
                          {searchSettingIsModified &&
                            "Setting is modified but not updated."}
                        </small>
                      </h5>
                    )}
                  </div>
                ...
                <div className="row">
                  <div className="col-sm-11 adv-search-label-holder">
                    <label
                      htmlFor="adv-s-search-in-select"
                      title="Search based on specific Metadata such as label or description."
                    >
                      Search in metadata
                      <i
                        className="fa fa-question-circle tooltip-questionmark"
                        aria-hidden="true"
                      ></i>
                    </label>
                  </div>
              ...
              <div className="col-sm-12">
                <div className="row">
                  <div className="col-sm-12 adv-search-label-holder">
                    <label
                      htmlFor="adv-s-search-under-term"
