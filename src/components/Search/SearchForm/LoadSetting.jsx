...
const LoadSetting = (props) => {
  ...
  function loadSetting() {
    let settingId = document.querySelector(".search-setting-checkbox:checked")
      ?.dataset?.id;
  ...
  async function deleteSetting() {
    ...
    if (deleted) {
      ...
      if (
        appContext.userSettings.activeSearchSetting["id"] ==
        settingToDelete["id"]
      ) {
        resetAdvancedSearch();
      }
  ...
  async function updateTitleAndDescription() {
    ...
    if (response) {
      setEditSuccess(true);
      if (
        appContext.userSettings.activeSearchSetting["id"] == settingToEdit["id"]
      ) {
  ...
  function fetchSettingList() {
    ...
    fetchSearchSettings().then((settingsList) => {
      setSettingsList(
        Toolkit.sortListOfObjectsByKey(settingsList, "created_at"),
      );
      let userSettings = appContext.userSettings;
      if (
        userSettings.activeSearchSetting &&
        userSettings.activeSearchSetting["id"]
      ) {
        let activeCheckbox = document.getElementById(
          "searchSettingSwitch" + userSettings.activeSearchSetting["id"],
        );
        if (activeCheckbox) {
  ...
  return (
    <>
      ...
      <Modal show={modalShow} id={"SearchSettingListModal"}>
        <Modal.Header>
          {!editMode && !deleteMode && (
            <h5 className="modal-title" id={"SearchSettingListModalLabel"}>
              {"My search settings"}
            </h5>
          )}
          {deleteMode && !editMode && (
            <h5 className="modal-title" id={"SearchSettingListModalLabel"}>
              {"Delete: " + settingToDelete["title"]}
            </h5>
          )}
          {editMode && !deleteMode && (
            <h5 className="modal-title" id={"SearchSettingListModalLabel"}>
              {"Edit: " + settingToEdit["title"]}
            </h5>
          )}
        </Modal.Header>
        <Modal.Body>
