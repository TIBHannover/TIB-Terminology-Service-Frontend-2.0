...
export function buildNoteAboutPart(note: Note | Record<string, any>) {
  let url =
    process.env.REACT_APP_PROJECT_SUB_PATH + "/ontologies/" + note.ontology_id;
  let label = "";
  if (note.semantic_component_type === "ontology") {
    ...
    url +=
      "/individuals?iri=" + encodeURIComponent(note.semantic_component_iri);
    label = note.semantic_component_label;
  }
...
export const PinnModalBtn = (props: PinnModalBtnProps) => {
  ...
  if (
    parseInt(String(noteContext.numberOfPinned)) ===
      parseInt(process.env.REACT_APP_MAX_PIN_NOTES ?? "0") &&
    !props.note.pinned
  ) {
    return null;
  }
      ...
      <Modal show={showModal} id={"pinModal" + props.modalId}>
        <Modal.Header className="row">
          <div className="col-sm-6">
            <h5 className="modal-title" id={"pinModalLabel" + props.modalId}>
              Confirmation
            </h5>
          </div>
          <div className="col-sm-6 text-end">
            {!submited && (
              <button
                type="button"
                className="close close-btn-message-modal"
        ...
        <Modal.Body>
          {!submited && (
            <span>
              Are you sure you want to {props.note.pinned ? "Unpin" : "Pin"}{" "}
              this note?
            </span>
          )}
        </Modal.Body>
