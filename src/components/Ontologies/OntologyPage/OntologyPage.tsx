...
const OntologyPage = (props: CmpPropp) => {
  ...
  let language =
    UrlFactory.getParam({ name: SiteUrlParamNames.Lang }) ||
  ...
  const [lastTabsStates, setLastTabsStates] = useState<{ [key: string]: any }>({
    terms: null,
    properties: null,
    gitIssues: "",
  });
  ...
  async function setCountOfNotes() {
    ...
    if (process.env.REACT_APP_NOTE_FEATURE === "true") {
      ...
      countOfNotes = notesList?.stats.total_number_of_records
        ? Number(notesList.stats.total_number_of_records)
        : 0;
    }
  ...
  function setTabOnLoad() {
    ...
    let activeTabId = TAB_ID_MAP_TO_TAB_ENDPOINT[requestedTab]
      ? TAB_ID_MAP_TO_TAB_ENDPOINT[requestedTab]
      : OVERVIEW_TAB_ID;
  ...
  if (error) {
    ...
    return (
      <div
        className="justify-content-center ontology-page-container"
        id="content-for-fullscreen"
      >
