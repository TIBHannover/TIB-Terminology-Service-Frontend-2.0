...
export const OntologyPageTabs = (props) => {
  ...
  */
  ...
  for (let configItemKey in props.tabMetadataJson) {
    ...
    let activeClassName =
      props.activeTabId === parseInt(configObject["tabId"])
        ? "nav-item-active"
        : "";
    result.push(
      <div
        className={
          "ontology-detail-nav-item " +
          activeClassName +
          " stour-" +
          configObject["id"]
        }
  ...
  return (
    <>
      <div className="row h-100">
        <div className="col-12 d-flex flex-column">{result}</div>
      </div>
    </>
  );
};
...
function collpaseSiteHeader() {
  ...
  let collapseIcone = document.getElementsByClassName(
    "header-collpase-icon",
  )[0];
  let collapseIconContainer = document.getElementsByClassName(
