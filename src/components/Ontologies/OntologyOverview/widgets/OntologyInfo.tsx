...
const OntologyInfoTable = () => {
  ...
  const [showExtraAnotationBtnText, setShowExtraAnotationBtnText] = useState(
  ...
  async function createAnnotations() {
    ...
    for (let prop in ontology.annotations) {
      ...
      if (prop.includes("contributor")) {
        valuesAsElements = await OntologyLib.formatCreator(
          valuesAsElements as string[],
        );
      }
      annotations.push(
        <tr>
          <td className="node-metadata-label">
            <b>{prop}</b>
          </td>
  ...
  function createImports(ontologiesIds: string[]) {
    ...
    for (let ontoId of ontologiesIds) {
      res.push(
        <a
          href={
            process.env.REACT_APP_PROJECT_SUB_PATH + "/ontologies/" + ontoId
          }
          className="me-2"
          target="_blank"
          ...
          <table className="table table-striped ontology-detail-table stour-overview-page-table">
            <tbody>
              ...
              <tr>
                ...
                <td>
                  {!creators ? <div className="isLoading-btn"></div> : creators}
                </td>
              </tr>
              <tr>
                <td className="ontology-overview-table-id-column">
                ...
                <td>{createImports(ontology.importsFrom)}</td>
              </tr>
              ...
              {process.env.REACT_APP_PROJECT_ID === "general" && (
                <tr>
                  ...
                  <td>{createCollections(ontology.collections)}</td>
                </tr>
              )}
              {process.env.REACT_APP_PROJECT_ID === "general" && (
                <tr>
                  ...
                  <td>{createSubjects(ontology.subjects)}</td>
                </tr>
              )}
              <tr>
                <td className="ontology-overview-table-id-column">
              ...
              <tr>
                ...
                <td>
                  ...
                  */}
                  ...
                  >
                    <i className="fa fa-download"></i>Ontology metadata as JSON
                  </a>
                </td>
              </tr>
            </tbody>
  ...
  useEffect(() => {
    OntologyLib.formatCreator(ontology.creator).then((resp: ReactElement) =>
      setCreators(resp),
    );
    createAnnotations();
  }, []);
    ...
    <div>
      ...
      {showExtraAnotation && (
        <table className="ontology-detail-table">
          <tbody>
            <tr>
              <td colSpan={3} id="annotation-heading">
                <b>Additional information from Ontology source</b>
              </td>
            </tr>
            {annotations}
          </tbody>
      ...
      <div className="text-center " id="search-facet-show-more-ontology-btn">
        <a
          className="show-more-btn stour-overview-page-more-metadata"
          onClick={handleOntologyShowMoreClick}
        >
          {showExtraAnotationBtnText}
        </a>
      </div>
    </div>
  );
