...
export const OntologyListRender = (props: CmpProps) => {
  ...
  function BuildCollectionForCard(collections: string[]) {
    ...
    for (let i = 0; i < collections.length; i++) {
      if (i !== collections.length - 1) {
        result.push(
          <span className="ontology-collection-name">
            <Link
              to={
  ...
  function createOntologyList() {
    ...
    for (let i = 0; i < props.ontologies.length; i++) {
      ...
      ontologyList.push(
        props.ontologiesHiddenStatus[i] ? (
          buildOntologyCard(item, i)
        ) : (
          <div></div>
        ),
      );
      ...
      <div className="col-sm-12">
        <div className="row">
          <div className="col-sm-6">
            <h3 className="h-headers stour-onto-list-browse">
              Browse Ontologies ({props.ontologies.length})
            </h3>
          </div>
        </div>
          ...
          <>
            <div className="row">
              <div className="col-sm-12">
                ...
                <Link
                  className="btn btn-sm btn-secondary ms-2 pt-0 pb-0 ps-1 pe-1 me-2 stour-onto-list-suggest-onto"
                  to={
                    process.env.REACT_APP_PROJECT_SUB_PATH +
                    "/ontologysuggestion"
                  }
        ...
        <div className="row mb-4">
          <div className="col-sm-12">{props.filterTags}</div>
        </div>
        ...
        <div className="row">
          <div className="col-sm-12">{ontologyListContent}</div>
        </div>
        <Pagination
