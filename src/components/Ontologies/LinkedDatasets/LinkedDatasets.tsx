...
import {
  DatasetLink,
  GetDatasetLinkServiceResp,
} from "../../../api/types/datasetLinks";
...
const LinkedDatasets = (props: CmpProps) => {
  ...
  groupByFromUrl =
    groupByFromUrl && ["term", DEFAULT_GROUP_BY].includes(groupByFromUrl)
      ? groupByFromUrl
      : DEFAULT_GROUP_BY;
  let sizeFromUrl =
    urlFactory.getParam({ name: SiteUrlParamNames.Size }) ?? DEFAULT_PAGE_SIZE;
  let pageFromUrl = urlFactory.getParam({ name: SiteUrlParamNames.Page }) ?? 1;
  let repositoryFromUrl = urlFactory.getParam({
    name: SiteUrlParamNames.Repository,
  });

  const [datasetLinksMap, setDatasetLinksMap] = useState<
    Map<string, DatasetLink[]>
  >(new Map());
  ...
  const [totalDatasetLinksCount, setTotalDatasetLinksCount] =
    useState<number>(0);
  const [reposReady, setReposReady] = useState<boolean>(false);

  const sizeOptions: DropDownOption[] = [
    { value: DEFAULT_PAGE_SIZE, label: String(DEFAULT_PAGE_SIZE) },
    {
      value: "30",
      label: "30",
    },
  ...
  const groupByOptions: DropDownOption[] = [
    { value: DEFAULT_GROUP_BY, label: "Dataset" },
    {
      value: "term",
      label: "Term",
    },
  ...
  function renderDatasetTableEntry(
    datasetTitle: string,
    ...
    return (
      <>
        <a
          href={`${process.env.REACT_APP_NFDI4CHEM_SEARCH_SERVICE_URL}/dataset/${datasetTitle}`}
          target="_blank"
  ...
  function createTermLinkHref(curie: string) {
    let targetHref =
      ...
      "/ontologies/" +
      encodeURIComponent(ontologyPageContext.ontology.ontologyId) +
      "/terms?curie=";
  ...
  function renderByDataset() {
    ...
    for (let [datasetTitle, dls] of datasetLinksMap) {
      ...
      results.push(
        <tr>
          <td className="col-6">
            {renderDatasetTableEntry(datasetTitle, datasetDescription)}
          </td>
          {!inputCurie && (
            <td className="col-6">
              {dls.map((dl: DatasetLink) =>
                renderCurieTableEntry(dl.curie!, dl.term_label!),
              )}
            </td>
          )}
        </tr>,
        ...
        <tr>
          ...
          <td className="col-6">
            {dls.map((dl: DatasetLink, index: number) => (
              <div
                key={index}
                className={index + 1 > truncateLimit ? "d-none" : ""}
              >
            ...
            {dls.length > truncateLimit && (
              <a
                href={createTermLinkHref(curie) + "&subtab=linked_datasets"}
                target="_blank"
                rel="noopener noreferrer"
  ...
  function handleGroupByChange(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    setPage(1);
    urlFactory.setParam({
      name: SiteUrlParamNames.GroupBy,
      value: newGroupBy,
      updateUrl: true,
    });
  }
  ...
  function filterByRepo(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    if (repoId) {
      urlFactory.setParam({
        name: SiteUrlParamNames.Repository,
        value: datasetRepos.find(
          (repo: DropDownOption) => repo.value === repoId,
        )?.label as string,
        updateUrl: true,
      });
    } else {
  ...
  function fetchData() {
    ...
    let repo = datasetRepos.find(
      (repo: DropDownOption) => repo.value === selectedRepo,
    );
    getDatasetLinks({
  ...
  function loadDatasetRepositoriesOptions() {
    ...
    setSelectedRepo(
      (reposOptions.find((op: DropDownOption) => op.label === repositoryFromUrl)
        ?.value as number) ?? 0,
    );
