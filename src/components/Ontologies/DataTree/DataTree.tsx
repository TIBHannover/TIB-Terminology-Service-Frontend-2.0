...
const DataTree = (props: DataTreeProps) => {
    ...
    */
    ...
    const paneResizeClass = useMemo(() => new PaneResize(), []);
    ...
    function handleTreeNodeSelection(selectedNodeIri: string, showDetailTable: boolean) {
    ...
    function handleJumtoSelection(selectedTerm: SelectedTerm | null) {
        if (selectedTerm) {
    ...
    useEffect(() => {
        ...
        if (iriInUrl) {
            ...
            setLoading(true);
            termApi.getTermIriByCurie(curieInUrl).then((iri: string) => {
