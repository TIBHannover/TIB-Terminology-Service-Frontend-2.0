  ...
  */
  ...
  function setTermRequestTemplate() {
    if (props.reportType === "termRequest") {
      fetch(templatePath)
        ...
        .then((text) => {
          setEditorState(
            EditorState.createWithContent(stateFromMarkdown(text)),
          );
        });
    }
  }
  ...
  function createIssueTemplatesDropDown() {
    ...
    for (let temp of templates) {
      result.push(
        <option value={value} key={value} url={temp["template_url"]}>
          {temp["template_name"]}
        </option>,
      );
      value += 1;
  ...
  function templateDropDownChange(e) {
    ...
    let selectedTemplateUrl =
      e.target.options[e.target.selectedIndex].getAttribute("url");
    ...
    fetch(selectedTemplateUrl)
      ...
      .then((templateText) => {
        setEditorState(
          EditorState.createWithContent(stateFromMarkdown(templateText)),
        );
        document.getElementsByClassName("rdw-editor-main")[0].style.border = "";
      });
  }
  ...
  return (
    <>
      <button
        type="button"
        className={
          "btn btn-secondary issue-report-btn stour-github-issue-open-btn-" +
          props.reportType
        }
        onClick={openModal}
      >
        {props.reportType === "termRequest"
          ? "File a Term Request"
          : "File a General Issue "}
      </button>
      <Login isModal={true} showModal={loginModal} withoutButton={true} />
      <Modal
        show={modalIsOpen}
        fullscreen={true}
        id={props.reportType + "_issue_modal"}
      >
          ...
          <div className="row">
            ...
            <div className="col-sm-6 text-end">
              <button
                onClick={closeModal}
                type="button"
                className="close close-mark-btn"
                data-dismiss="modal"
              >
                &times;
              </button>
            ...
            <>
              {props.reportType === "general" && (
                <div className="alert alert-info">
                  <strong>Note:</strong> Please select a proper issue template
                  (if exist). These templates are defined by the repository
                  owner and are the expected way of reporting an issue.
                </div>
              )}
              <div className="row">
                  ...
                  {props.reportType === "general" && (
                    <div className="form-group">
                      ...
                      <select
                        ...
                        onChange={templateDropDownChange}
                      >
            ...
            <div className="row text-center">
              <div className="col-sm-12">
                <div className="alert alert-success">
                  ...
                  <br></br>
                  You can find all the submitted term requests and issues in
                  your profile:
                  <a
                    href={
