      ...
      removeTermFromSet(termsetId, termId).then((removed) => {
        if (removed) {
          ...
          if (termsetInContextIndex > 0) {
            let tindexInSet = usertermsets[
              termsetInContextIndex
            ].terms.findIndex(
      ...
      <div className="tree-view-container list-container">
        ...
        <div className="row bg-white p-2 rounded termset-detail-header">
          <div className="col-sm-8">
            ...
            <small>
              {data
                ? data.created_at.split("T")[0] + " by " + data.creator_name
                : ""}
            </small>
            ...
            <div className="d-flex flex-row gap-2  justify-content-end">
              ...
              {appContext.user &&
                appContext.userTermsets.find(
                  (tset) => tset.id === data?.id,
                ) && (
                  // only owner can see this button
                  <div>
                    <AddTermModal
                      termset={data ?? undefined}
                      modalId={"add-term-modal"}
                    ></AddTermModal>
                  </div>
                )}
            </div>
