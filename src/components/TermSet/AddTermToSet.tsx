...
export const AddToTermsetModal = (props: AddToTermsetModalComProps) => {
  ...
  function addTermToSet() {
    ...
    let selectedTermsetsContainer = document.getElementById(
      "selected-termsets-" + term?.iri,
    );
    if (
      selectedTermsetIds.length === 0 &&
      selectedTermsetsContainer?.style?.border
    ) {
  ...
  function removeTerm(e: React.MouseEvent<HTMLElement>) {
    try {
      ...
      removeTermFromSet(termsetId, term.iri).then((removed) => {
        if (removed) {
          let deletedSet = document.getElementById(
            "termCurrentSetsLi-" + termsetId,
          );
          if (deletedSet) {
            let existingSetsList = [...termExistingSets];
            let userTermsets = [...appContext.userTermsets];
            let setToRemoveIndex = existingSetsList.findIndex(
            ...
            let targetSetIndex = userTermsets.findIndex(
              (tset) => tset.id === termsetId,
            );
            let termToRemoveIdex = userTermsets[targetSetIndex].terms.findIndex(
              (tsetTerm: OntologyTermDataV2) => tsetTerm.iri === term.iri,
            );
  ...
  function renderTermsetCreateSection() {
    ...
    return [
      <>
        ...
        <div className="row">
          <div className="col-sm-12">
            <label htmlFor={"termsetDescription" + modalId}>
  ...
  function renderTermsetAddingSection() {
        ...
        <div className="row">
          <div className="col-sm-12">
            <AlertBox
              type="info"
              message={
                "Would you like to create your own term set? Log in to get started!"
              }
            ></AlertBox>
    ...
    return [
      <>
        <h6>Please select or create a set to add this term.</h6>
        <br></br>
        <Multiselect
          isObject={true}
          options={termsets}
            ...
            {termExistingSets.map((tset) => {
              return (
                <li id={"termCurrentSetsLi-" + tset.id}>
                  ...
                  {appContext.user && removeLoading.get(tset.id) && (
                    <div className="isLoading-inline-small"></div>
                  )}
                  {appContext.user && (
                    <span className="">
                      <i
  ...
  let title =
    "Add this term to your termsets. Check the existing termsets for this term.";
  if (!appContext.user) {
      ...
      <Modal show={showModal} id={"addToTermsetModal-" + modalId}>
        <Modal.Header className="row">
          <div className="col-sm-10">
            <h5 className="modal-title" id={"addToTermsetModal-" + modalId}>
              {modalTitle}
            </h5>
          </div>
