...
class Toolkit {
  ...
  static sortListOfObjectsByKey(
    objectList: Array<GenericObject>,
    key: string,
    ...
    if (parentKey) {
      return objectList.sort(function (a: GenericObject, b: GenericObject) {
        let x: number | string =
          typeof a[key] === "number"
            ? a[parentKey][key]
            : a[parentKey][key]?.toLowerCase();
        let y: number | string =
          typeof b[key] === "number"
            ? b[parentKey][key]
            : b[parentKey][key]?.toLowerCase();
        return x < y ? reverseSortSign : -1 * reverseSortSign;
      });
    }
    return objectList.sort(function (a: GenericObject, b: GenericObject) {
      let x: number | string =
        typeof a[key] === "number" ? a[key] : a[key]?.toLowerCase();
      let y: number | string =
        typeof b[key] === "number" ? b[key] : b[key]?.toLowerCase();
      return x < y ? reverseSortSign : -1 * reverseSortSign;
    });
  }
  ...
  static getObsoleteFlagValue() {
    ...
    if (!currentUrlParams.get("obsoletes")) {
      let obsoleteValue: any = Toolkit.getVarInLocalSrorageIfExist(
        "obsoletes",
        "false",
      );
    ...
    let obsoleteValue =
      currentUrlParams.get("obsoletes") === "true" ? true : false;
