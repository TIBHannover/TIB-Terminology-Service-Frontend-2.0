 ...
 var LoadingAnimation=true
   ...
   if (networkOptions.displayOptions.showInfoWindow===true)
    ...
    function initializeIButtonBox(){
       ...
       if (networkOptions.displayOptions.showInfoWindow){ 				   showNodeInfo(nodeToFocus); }
            }
       ...
       if (networkOptions.displayOptions.showInfoWindow){    showClusterInfo(tmpClusterNode);    }
            }
...
//Go through all Nodes of the network, check each if it is a cluster and if so release the cluster
     ...
     */
  function unclusterSingleCluster(id){
...
return  x=tmpstring.replace("/","_");
    ...
    function fetchGraphData(linkToGraph){
        ...
        jQuery.getJSON(linkToGraph,function (data){
           ...
           //If the node is not part of the root array, then add it now (collapsed if they get reexpanded might already be part of the root array)
           ...
           if (data.nodes.length>=networkOptions.dataOptions.maximumNodesPerWebserviceCall)
               printMsgToScreen("For performance reasons the webservice 'only' returns up to "+networkOptions.dataOptions.maximumNodesPerWebserviceCall+" nodes! So there might nodes be missing in this representation!")
   ...
   //After startup, select a Node to highlight and update InfoWindow with this node
    ...
    function onDoubleClick(params)
    {
        ...
        if (typeof params.nodes[0]!=="undefined")
        {
            ...
            {unclusterSingleCluster(params.nodes[0]);}
   ...
   if (networkOptions.displayOptions.showInfoWindow){
           if (network.isCluster(params.nodes[0]))
...
// If the relationshiptype is set to true, add the view from removedNodes to the node dataset (the one that is displayed in the network)
 ...
 /*
 * Function cleanUpRelationshipLabels
 * -This function gets rid of special characters in relationship labels. This is necessary because otherwise it breaks the legend
 */
 function cleanUpRelationshipLabels(label){
   var cleanLabel=label.replace(/\s+/g, '')
...
//After appending the legend, the colours have to be adjusted according to the colorMap
 ...
 visualisation.prototype.unclusterANode=function(clusterId){ unclusterSingleCluster(clusterId); };
 visualisation.prototype.getGraphDataset=function(){return graphdataset}
 visualisation.prototype.getNetwork=function(){return network}
